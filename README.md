...
These playbooks install and configure Cyware agent, manager and indexer and dashboard.
...
- [Cyware Ansible documentation](https://documentation.cyware.khulnasoft.com/current/deploying-with-ansible/index.html)
- [Full documentation](http://documentation.cyware.khulnasoft.com)
...
## Directory structure
    ...
    │ ├── playbooks
    │ │ ├── cyware-agent.yml
    │ │ ├── cyware-dashboard.yml
    │ │ ├── cyware-indexer.yml
    │ │ ├── cyware-manager-oss.yml
    | | ├── cyware-production-ready
    │ │ ├── cyware-single.yml
    │
    │ ├── README.md
    │ ├── VERSION
...
The hereunder example playbook uses the `cyware-ansible` role to provision a production-ready Cyware environment. The architecture includes 2 Cyware nodes, 3 Cyware indexer nodes, and a Cyware dashboard node.
...
```bash
sudo ansible-playbook cyware-production-ready.yml -i inventory
```

After the playbook execution, the Cyware UI should be reachable through `https://<dashboard_host>`

## Example: single-host environment

### Playbook

The hereunder example playbook uses the `cyware-ansible` role to provision a single-host Cyware environment. This architecture includes all the Cyware and Opensearch components in a single node.
...
```bash
sudo ansible-playbook cyware-single.yml -i inventory
```

After the playbook execution, the Cyware UI should be reachable through `https://<your server host>`

## Example: Cyware server cluster (without Filebeat)

### Playbook

The hereunder example playbook uses the `cyware-ansible` role to provision a Cyware server cluster without Filebeat. This architecture includes 2 Cyware servers distributed in two different nodes.
...
NOTE: `hosts` and `cyware_manager_config.cluster_node_name` are the only parameters that differ from the `worker01` configuration.
...
```bash
sudo ansible-playbook cyware-manager-oss-cluster.yml -i inventory
```
...
If you are not familiar with Github, you can also share them through [our users mailing list](https://groups.google.com/d/forum/cyware), to which you can subscribe by sending an email to `cyware+subscribe@googlegroups.com`.
...
CYWARE
Copyright (C) 2016, KhulnaSoft Ltd.  (License GPLv2)

## Web references

- [Cyware website](http://cyware.khulnasoft.com)
